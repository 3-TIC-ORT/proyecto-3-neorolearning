...
onEvent("traerPalabras", getPalabras);

function getPalabras(juego, nivel) {
	let data = JSON.parse(fs.readFileSync("palabras.json", "utf8"));
	let palabras = data[juego][nivel];

	let unusedWordGroups = Object.values(palabras).filter(
		(grupo) => grupo[grupo.length - 1]["usada"] === "no"
	);

	if (!unusedGroupsRemain(unusedGroupsRemain)) {
		return { msg: `Nivel finalizado: ${nivel}` };
	}

	let group = getRandomWordGroup(unusedGroupsRemain, data)
	
	return group
}

function getRandomWordGroup(unusedGroups, data) {
	let randomGroup = unusedGroups[Math.floor(Math.random() * unusedGroups.length)];
	// after getting a random group- set to "used"
	randomGroup[randomGroup.length - 1]["usada"] = "si";
...
function unusedGroupsRemain(groups) {
	if (groups.length === 0) {
		console.log("No hay palabras disponibles para este nivel. LOU");
		return false;
	}
